ques = ['''hcpjssql4kjhbcqzkvr2fivebpllzqbkhg
...
for i in new:
    ...
    i_rev = i[::-1]
    while j <= len(i_rev):
        if i_rev[j].isnumeric():
            num_list_dig.append([len(i)-1-j, i_rev[j]])
            break
        j+=1
    ...
    num_list_let = []
    while j <= len(i):
        if i[j:j+3] in numbers:
            num_list_let.append([j, i[j:j+3]])
            
        if i[j:j+4] in numbers:
            num_list_let.append([j, i[j:j+4]])
            
        if i[j:j+5] in numbers:
            num_list_let.append([j, i[j:j+5]])

        j+=1
   
    for i in range(len(num_list_let)):
        text = num_list_let[i][1]
        num_list_let[i][1] = num_dict[text]
    for i in range(len(num_list_dig)):
        num_list_dig[i][1] = int(num_list_dig[i][1])

    num_list = num_list_dig+num_list_let
    ...
    final = int(str(min_num) + str(max_num))
    ans_list.append(final)
print(sum(ans_list))

#     if num_set != final:
#         # print(num_set, final)
#         # print(grand_fuck_of_numbers)
#         # print(num_list)
#         # print(min_num, max_num)
#         # print()
#         total_failures += 1

# print(total_failures/1000)
# print((1000 - total_failures)/1000)
